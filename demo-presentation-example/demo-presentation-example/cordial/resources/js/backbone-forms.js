...
(function(root) {
    ...
    var Form = (function() {
        ...
        return Backbone.View.extend({
                ...
                initialize: function(options) {
                    ...
                    //extending events
                    $.extend(self.events, options.customEvents);
                    //Option defaults
                    options = _.extend({
                    ...
                    if (options.isModal) {
                        options.template = 'modalform';
                    }

                    if (options.extraPostVals) {
                        this.extraPostVals = options.extraPostVals;
                    }
                    ...
                    if (options.saveOverride) {
                        this.saveOverride = options.saveOverride;
                    }
                    if (options.template) {
                    ...
                    this.hasReadOnly = options.hasReadOnly;
                    this.startReadOnly = options.startReadOnly;
                ...
                render: function() {
                    ...
                    var self = this,
                        ...
                        template = Form.templates[this.template];      
                    ...
                    if (this.hasReadOnly && this.startReadOnly) {
                        this.makeReadonly();
                        madeReadonly = true
                    }
                ...
                renderFieldset: function(fieldset) {
                    ...
                    //Set footer
                    if (fieldset.footer) {
                        $fieldset.find('.bbf-tmp-footer').replaceWith(fieldset.footer);
                    }
                    //or remove the containing tag if there isn't a footer
                    else {
                        $fieldset.find('.bbf-tmp-footer').parent().remove();
                    }
                    //----------
                ...
                */
                ...
                getModelValues: function() {
                    ...
                    var values = {};
                    _.each(this.fields, function(field) {
                        self.setModelObjVal(field.key, field.getValue(), values);
                    });

                    return values;
                },

                setModelObjVal: function(name, value, context) {
                ...
                setup: function(selector, options) {
                    ...
                    this.$el.bind("keyup keypress", function(e) {
                ...
                readOnlySuccess: function(self) {
                    $(self.selector).html(self.render().el);
                    self.makeReadonly();
                },

                makeReadonly: function() {
                    tinymce.remove('textarea', $(this.el));
                    this.$('.editmode').hide();
                    $('.readmode', $(this.el)).show();
                },

                makeEditable: function() {
                    console.log("make editable", this.selector);
                    console.log($(this.selector));
                    $(this.selector).html(this.render().el);

                    $('.editmode', $(this.el)).show();
                    $('.readmode', $(this.el)).hide();
                    $('.formAlerts', $(this.el)).hide();
                ...
                cancelForm: function(e) {
                    ...
                    if (this.cancelCallback) {
                        ...
                        $.each(this.schema, function(index, val) {
                            fieldval = model.get(index);
                            if (typeof fieldval === 'undefined') {
                                fieldval = '';
                            }

                            thisform.setValue(index, fieldval);

                            thisform.fields[index].clearError();
                        });
                        if (this.hasReadOnly) {
                            this.makeReadonly();
                        }
                    }
                    this.trigger("on:cancel:success");
                },
                ...
                saveForm: function(e) {
                    ...
                    if (this.saveOverride) {
                        ...
                        this.saveOverride(this.model);
                        return;
                    }

                    var modelData = this.getModelValues();
                    if (this.extraPostVals) $.extend(modelData, this.extraPostVals);
                    this.model.save(modelData, {
                        ...
                        success: function(model, response) {
                            if (self.successCallback) {
                                ...
                                if (self.hasReadOnly) {
                                    self.readOnlySuccess(self);
                                } else {
                                    $.each(self.schema, function(index, val) {
                                        self.setValue(index, '');
                                    });
                                }
                                var message = 'Your changes were saved';
                                if (self.successMessage) {
                                    message = self.successMessage;
                                }

                                $('.formAlerts', self.$el).html('<div class="alert alert-success"><a class="close" data-dismiss="alert">x</a><span>' + message + '</span></div>');
                                $('.formAlerts', self.$el)
                                    .delay(200)
                                    .fadeIn()
                                    .delay(4000)
                                    .fadeOut();
                                self.trigger("on:save:success", response);
                            }
                        },
                        error: function(model, response) {
                            ...
                            // apply serverside validation errors
                            if (responseObj.fieldErrors) {
                                _.each(responseObj.fieldErrors, function(error, key) {
                                    this.fields[key].setError(error);
                                    hasFieldError = true;
                                }, self);
                            }

                            if (!responseObj.message && !hasFieldError) {
                                responseObj.message = 'There was an error saving your changes.';
                            }

                            if (responseObj.message) {
                                $('.formAlerts', self.$el).html('<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>' + responseObj.message + '</div>');
                                $('.formAlerts', self.$el).show();
                            }
                        }
                    });
                ...
                inputAttributeName: function(e) {
                    this.options.inputAttributeName(e, this);
                },

                events: {
                    "click .editReadOnly": 'makeEditable',
                    "click .readOnlyCancel": 'cancelForm',
                    "click .readOnlySave": 'saveForm'
                }
    ...
    Form.helpers = (function() {
        ...
        helpers.createReadonly = function(schemaType, options) {
            var constructorFn;
            if (_.isString(schemaType)) {
                constructorFn = Form.readonly[schemaType];
            } else {
                constructorFn = schemaType;
            }
    ...
    Form.Field = (function() {
        ...
        return Backbone.View.extend({
            ...
            initialize: function(options) {
                ...
                this.model = options.model;
                options.idPrefix = this.model.cid; 
                //Turn schema shorthand notation (e.g. 'Text') into schema object
                if (_.isString(options.schema)) options.schema = {
                    type: options.schema
                };
            ...
            render: function() {
                ...
                //Render editor
                var editorField = editor.render().el;
                $(editorField).addClass('editmode');
                $field.find('.bbf-tmp-editor').replaceWith($(editorField));

                //Set the readonly content
                if (this.form.hasReadOnly) {
                    var readonly = helpers.createReadonly(schema.type, options);
                    $field.find('.bbf-tmp-readonly').replaceWith(readonly.render().el);
                    //$('.readContent', $field).html(readonly.render().el);
                }
    ...
    Form.editors = (function() {
        ...
        editors.Text = editors.Base.extend({
            ...
            initialize: function(options) {
                ...
                this.$el.attr('placeholder', schema.placeHolder);
            },
        ...
        //URL
        editors.Url = editors.Text.extend({
        ...
        editors.TextArea = editors.Text.extend({
            ...
            initialize: function(options) {
                ...
                if (this.schema.wysiwyg) this.$el.addClass('wysiwyg');
            },

            getValue: function() {
                if (this.schema.wysiwyg) {
                    return tinyMCE.get(this.$el.attr('id')).getContent();
                } else {
                    return this.$el.val();
                }
            },
        ...
        editors.Select = editors.Base.extend({
            ...
            render: function() {
                ...
                if (typeof this.schema.multiple != 'undefined') {
            ...
            _arrayToHtml: function(array) {
                ...
                _.each(array, function(option) {
                    if (_.isObject(option)) {
                        ...
                        } else if (option.id || option.id === 0) { // adding select to options format
                            var val = option.id;
                            var label = option.text;
                        } else {
                            var val = '';
                            var label = (option.label) ? option.label : option.text;
                        }

                        html.push('<option value="' + val + '">' + label + '</option>');
                    } else {
        ...
        editors.Checkboxes = editors.Select.extend({
            ...
            _arrayToHtml: function(array) {
                ...
                _.each(array, function(option, index) {
                    ...
                    if (_.isObject(option)) {
                        var val = (option.val || option.val === 0) ? option.val : '';
                        itemHtml += ('<div class="checkbox"><label><input type="checkbox" name="' + self.id + '" value="' + val + '" id="' + self.id + '-' + index + '" /> ' + option.label + '</label></div>');
                    } else {
                        itemHtml += ('<div class="checkbox"><label><input type="checkbox" name="' + self.id + '" value="' + option + '" id="' + self.id + '-' + index + '" /> ' + option + '</label></div>');
                    }
                    itemHtml += '</li>';
                    html.push(itemHtml);
                });
    ...
    Form.readonly = (function() {

        var helpers = Form.helpers;

        var readonly = {};

        readonly.Base = Backbone.View.extend({

            tagName: 'p',
            className: 'form-control-static readmode',
        ...
        readonly.Text = readonly.Base.extend({
        ...
        readonly.Number = readonly.Text.extend({});

        readonly.Password = readonly.Text.extend({
        ...
        readonly.Url = readonly.Text.extend({
            render: function() {
                var value = '<a href="' + this.value + '" target="_blank">' + this.value + '</a>';
                this.setValue(value);
                this.$el.addClass('truncate-readonly-value');
                return this;
            }
        });

        readonly.TextArea = readonly.Text.extend({});

        readonly.Checkbox = readonly.Text.extend({

            render: function() {

                if (this.value) {
                    this.setValue('<span class="label label-success">Yes</span>');
                } else {
                    this.setValue('<span class="label label-danger">No</span>');
                }
        ...
        readonly.Select = readonly.Base.extend({
            ...
            render: function() {
                ...
                if (this.schema.remoteData) { // this isn't done but will be needed for larger data sets
                    val = this.value;
                } else if (_.isArray(options)) { // If array
                    if (_.isObject(options[0])) {
                        var selected = _.detect(options, function(obj) {
                            return (obj.val === curval || obj.id === curval)
                        });
                        if (selected.text) {
                        ...
                        if ($.inArray(curval, options) > -1) {
                            this.setValue(curval);
                        }
                    }
                } else if (options instanceof Backbone.Collection) { //Or Backbone collection
                    var collection = options;
                    //console.log(options);
                    //console.log(this.value);
                    //Don't do the fetch if it's already populated
                    if (collection.length > 0) {
                        val = collection.get(this.value);
                        if (typeof val != 'undefined') {
                            self.setValue(val.toString());
                        }
                    } else {
                        if (this.schema.query) {
                            var data = this.schema.query;
        ...
        readonly.Radio = readonly.Select.extend({

        });

        readonly.Checkboxes = readonly.Select.extend({
            ...
            render: function() {
                ...
                //Accept string of HTML
                if (_.isString(this.value)) { // assuming we won't have options as a string for the time being
                    html = this.value;
                }
            ...
            _arrayToHtml: function(array) {
                ...
                _.each(array, function(option, index) {
                    ...
                    if (_.isObject(option)) {
                        var val = (option.val || option.val === 0) ? option.val : '';
                        if (_.contains(self.value, val)) {
                            itemHtml += '<li>' + option.label + '</li>';
                        }
                    } else {
                        if (_.contains(self.value, option)) {
                            itemHtml += '<li>' + option + '</li>';
                        }
                    }
                    html.push(itemHtml);
                });

                return '<ul class="list-unstyled">' + html.join('') + '</ul>';
            },
        ...
        readonly.Object = readonly.Base.extend({
    ...
    Form.setTemplates({
        ...
        //HTML
        form: '\
      <form class="form-horizontal" role="form">{{fieldsets}}</form>\
    ',
      ...
      <form class="bbf-form">\
        ...
        <div class="form-actions">\
            <button class="btn btn-primary btn-sm filter-grid">Search</button>\
            or\
            <button class="btn btn-sm clear-grid">Clear</button>\
        </div>\
      </form>\
    ',
          ...
          <div class="modal-dialog">\
            <div class="modal-content">\
              ...
              <div class="modal-body">\
                <form class="form-horizontal"><div class="formAlerts"></div>{{fieldsets}}</form>\
              </div>\
              <div class="modal-footer"><a class="btn btn-primary ok">{{okText}}</a><a class="btn cancel">{{cancelText}}</a></div>\
            </div><!-- /.modal-content -->\
          </div><!-- /.modal-dialog -->\
        </div><!-- /.modal -->\
    ',
    ...
    ',
        ...
        <div class="form-group  field-{{key}}">\
            ...
            <% if (typeof(editor1.validators) != "undefined") { %>\
                <i class="fa fa-asterisk required"></i>\
            <% } %>\
            {{title}}</label>\
            <div class="bbf-editor col-sm-9">{{editor}}\
                <div class="bbf-tmp-readonly"></div>\
                <div class="help-block">{{help}}</div>\
                <div class="help-error has-error">{{error}}</div>\
            </div>\
        </div>\
    ',
      ...
      <li class="bbf-field bbf-nested-field field-{{key}}" title="{{title}}">\
        ...
        <div class="help-block">{{help}}</div>\
        <div class="help-error">{{error}}</div>\
      </li>\
    ',
    ...
    ',
      ...
      <div class="bbf-list-modal">\
        <span class="readContent readmode muted" style="display: inline;">\
        {{summary}}\
        </span>\
      </div>\
    ',
